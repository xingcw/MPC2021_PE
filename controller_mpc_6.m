...
if isempty(param)
    ...
    P = [0.99,0.98,0.25,0.97,0.03,0.02];
    L = (place(Aaug',Caug',P))';
    M = [param.A-eye(3),param.B;eye(3),zeros(3)];
end

% evaluate control action by solving MPC problem
[u_mpc,errorcode] = yalmip_optimizer(T,param.us);
if (errorcode ~= 0)
...
objective = 0;
for k = 1:N-1
    constraints = [constraints,(param.Pcons(:,1)-param.p_sp<=U{k})&(U{k}<=param.Pcons(:,2)-param.p_sp),(param.Tcons(:,1)-param.T_sp<=X{k+1})&(X{k+1}<=param.Tcons(:,2)-param.T_sp),X{k+1}==param.A*X{k}+param.B*U{k}];
    objective = objective +X{k}'*Q*X{k}+(U{k}-us)'*R*(U{k}-us);
end
constraints = [constraints, A_x*X{N}<=b_x];
objective = objective +  X{N}'*P_f*X{N};
ops = sdpsettings('verbose',0,'solver','quadprog');
yalmip_optimizer = optimizer(constraints,objective,ops,{T0,us},U);
end